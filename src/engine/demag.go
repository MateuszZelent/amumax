...
func setDemagField(dst *data.Slice) {
	if EnableDemag {
		msat := Msat.MSlice()
		defer msat.Recycle()
		if NoDemagSpins.isZero() && !Msat.hasZero() {
			// Normal demag, everywhere
			geom := Geometry.Gpu()
			demagConv().Exec(dst, NormMag.Buffer(), geom, msat)
			applyDemagBoundaryCorrection(dst, geom, msat)
		} else {
...
func setMaskedDemagField(dst *data.Slice, msat cuda.MSlice) {
	...
	applyDemagBoundaryCorrection(dst, buf, msat)

	// After convolution, mask-out the field in the NoDemagSpins or Msat=0 cells
