...
func SetHopfIndexDensity_SolidAngle(h, m *data.Slice, mesh mesh.MeshLike) {
	N := m.Size()
...
func SetEmergentMagneticField_SolidAngle(F, m *data.Slice, mesh mesh.MeshLike) {
	...
	kSetemergentmagneticfieldsolidangleAsync(F.DevPtr(X), F.DevPtr(Y), F.DevPtr(Z),
