...
func GetHopfIndex_SolidAngleFourier(m *data.Slice, mesh mesh.MeshLike) float64 {
	...
	cfg := make3DConf(N)
	kScaleemergentfieldAsync(F.DevPtr(X), F.DevPtr(Y), F.DevPtr(Z),
	...
	kSolidanglefourierfieldAsync(fftCBufX.DevPtr(0), fftCBufY.DevPtr(0), fftCBufZ.DevPtr(0),
		Fx_k.DevPtr(0), Fy_k.DevPtr(0), Fz_k.DevPtr(0),
		N[X], N[Y], N[Z], cfg)

	summand := NewSlice(1, N)
	kSolidanglefouriersummandAsync(summand.DevPtr(0), Fx_k.DevPtr(0), Fy_k.DevPtr(0), Fz_k.DevPtr(0), N[X], N[Y], N[Z], cfg)
